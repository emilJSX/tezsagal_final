...
}); function clearfunction() { $('.form-group span.close-icon').on('click', function () { $(this).parent().parent().hide(); }); }
$('.append-btn').on('click', function () {
    const services = $('#Services').val(); if (services.length) { $('#profileCard').append('<li><div class="form-group">' + services + '<span class="close-icon"></span></div></li>'); $('#data')[0].reset() }
    clearfunction();
}); $('.append-btn1').on('click', function () {
    const specialization = $('#Specialization').val(); if (specialization.length) { $('#profileCard1').append('<li><div class="form-group">' + specialization + '<span class="close-icon"></span></div></li>'); $('#data1')[0].reset() }
    clearfunction();
}); function f1() { $('ul li span.trash-icon').on('click', function () { $(this).parent().parent().hide(); }); }
var education_details = $('.education-experience form').html(); $('.education-experience span.add-btn').on('click', function () { $(".education-experience form").append(education_details); f1(); }); var work_details = $('.work-experience form').html(); $('.work-experience span.add-btn').on('click', function () { $(".work-experience form").append(work_details); f1(); }); var awards_details = $('.awards-experience form').html(); $('.awards-experience span.add-btn').on('click', function () { $(".awards-experience form").append(awards_details); f1(); }); var membership_details = $('.membership-experience form').html(); $('.membership-experience span.add-btn').on('click', function () { $(".membership-experience form").append(membership_details); f1(); }); var registration_details = $('.registration-col form').html(); $('.registration-col span.add-btn').on('click', function () { $(".registration-col form").append(registration_details); f1(); }); $(".add-hours").on('click', function () {
...
(function ($) {
    ...
    if ($('.datetimepicker').length > 0) { $('.datetimepicker').datetimepicker({ format: 'DD/MM/YYYY', icons: { up: "fas fa-chevron-up", down: "fas fa-chevron-down", next: 'fas fa-chevron-right', previous: 'fas fa-chevron-left' } }); }
    if ($('.yearpicker').length > 0) { $('.yearpicker').datetimepicker({ format: 'YYYY', icons: { up: "fas fa-chevron-up", down: "fas fa-chevron-down", next: 'fas fa-chevron-right', previous: 'fas fa-chevron-left' } }); }
    ...
    $('body').append('<div class="sidebar-overlay"></div>'); $(document).on('click', '#mobile_btn', function () { $('main-wrapper').toggleClass('slide-nav'); $('.sidebar-overlay').toggleClass('opened'); $('html').addClass('menu-opened'); return false; }); $(document).on('click', '.sidebar-overlay', function () { $('html').removeClass('menu-opened'); $(this).removeClass('opened'); $('main-wrapper').removeClass('slide-nav'); }); $(document).on('click', '#menu_close', function () { $('html').removeClass('menu-opened'); $('.sidebar-overlay').removeClass('opened'); $('main-wrapper').removeClass('slide-nav'); }); if ($('[data-bs-toggle="tooltip"]').length > 0) {
    ...
    if ($('.owl-carousel.report-card-slider').length > 0) { $('.owl-carousel.report-card-slider').owlCarousel({ loop: true, margin: 10, nav: true, navText: ["<i class='fa fa-chevron-left'></i>", "<i class='fa fa-chevron-right'></i>"], responsive: { 0: { items: 1 }, 600: { items: 1 }, 1000: { items: 1 } } }) }
    ...
    if ($('.header-trans-two').length > 0) {
        $(document).ready(function () {
            $(window).scroll(function () {
                var scroll = $(window).scrollTop(); if (scroll > 95) { $(".header-trans-two").css("background", "#E5F1FB"); }
                else { $(".header-trans-two").css("background", "#FFFFFF"); }
            })
        })
    }
    ...
    var $status = $('.slider-counter-three'); var $slickElement = $('.home-banner-three-slider'); $slickElement.on('init reInit afterChange', function (event, slick, currentSlide, nextSlide) {
        if (!slick.$dots) { return; }
        var i = (currentSlide ? currentSlide : 0) + 1; $status.text(i + '/' + (slick.$dots[0].children.length));
    ...
    $('.doctor-fav-btn .fav-icon').on('click', function () { $(this).toggleClass('favourite'); }); if ($('.best-doctor-slider').length > 0) { $('.best-doctor-slider').slick({ dots: false, autoplay: false, infinite: true, slidesToShow: 4, slidesToScroll: 1, responsive: [{ breakpoint: 992, settings: { slidesToShow: 3 } }, { breakpoint: 800, settings: { slidesToShow: 2 } }, { breakpoint: 776, settings: { slidesToShow: 2 } }, { breakpoint: 567, settings: { slidesToShow: 1 } }] }); }
    ...
    $('a.smooth-menu').on('click', function (e) { var $anchor = $(this); $('html, body').stop().animate({ scrollTop: $($anchor.attr('href')).offset().top }); e.preventDefault(); }); var window_width = $(window).outerWidth(); var container_width = $('.container').outerWidth(); var full_width = window_width - container_width
    var right_position_value = full_width / 2
    $('.banner-two-slider .slider-counter').css('right', right_position_value); $('.banner-two-slider .slick-next').css('right', right_position_value + 10); $('.banner-two-slider .slick-prev').css('right', right_position_value + 150); var window_width = $(window).outerWidth(); var container_width = $('.container').outerWidth(); var full_width = window_width - container_width
    var right_position_value = full_width / 2
    $('.home-three-slider .owl-dots').css('right', right_position_value + 43); $('.home-three-slider .owl-next').css('right', right_position_value + 10); $('.home-three-slider .owl-prev').css('right', right_position_value + 150); if ($('.onboard-slider').length > 0) { $('#onboard-slider').owlCarousel({ items: 1, margin: 30, dots: true, nav: true, navText: false, loop: true, smartSpeed: 450, autoplay: true, autoplayTimeout: 5000, autoplayHoverPause: true, responsiveClass: true, responsive: { 0: { items: 1 }, 768: { items: 1 }, 1170: { items: 1 } } }); }
    $('.digit-group').find('input').each(function () { $(this).attr('maxlength', 1); $(this).on('keyup', function (e) { var parent = $($(this).parent()); if (e.keyCode === 8 || e.keyCode === 37) { var prev = parent.find('input#' + $(this).data('previous')); if (prev.length) { $(prev).select(); } } else if ((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 65 && e.keyCode <= 90) || (e.keyCode >= 96 && e.keyCode <= 105) || e.keyCode === 39) { var next = parent.find('input#' + $(this).data('next')); if (next.length) { $(next).select(); } else { if (parent.data('autosubmit')) { parent.submit(); } } } }); }); $('.digit-group input').on('keyup', function () { var self = $(this); if (self.val() != '') { self.addClass('active'); } else { self.removeClass('active'); } }); $(".tog-button").click(function () { $(".tog-button span").animate({ right: '4px' }); }); function readURL(input) {
        if (input.files && input.files[0]) {
            var reader = new FileReader(); reader.onload = function (e) { $('#blah').attr('src', e.target.result); $('.upload-pic h6').hide(); }
            reader.readAsDataURL(input.files[0]);
        }
    }
    $("#imgInp").change(function () { readURL(this); }); $("#test1").change(function () { if ($(this).is(":checked")) { $('.pregnant-col').hide(); } }); $("#test2").change(function () { if ($(this).is(":checked")) { $('.pregnant-col').show(); } }); $("#radio2").change(function () { if ($(this).is(":checked")) { $("#change-url").attr("href", "onboarding-email.html"); } }); $("#radio1").change(function () { if ($(this).is(":checked")) { $("#change-url").attr("href", "onboarding-phone.html"); } }); $('#change-url').on('click', function (e) { if ($('#radio1').is(":not(:checked)") && $('#radio2').is(":not(:checked)")) { alert('please select any one verify method!!!'); } }); $("#document-name").click(function () { $('.upload-sucess-note h6:nth-child(2)').fadeOut().hide(400); });
})(jQuery);